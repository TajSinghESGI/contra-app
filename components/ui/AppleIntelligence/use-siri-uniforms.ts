...
function useSiriUniforms<T extends IUseSiriUniformsOptions>({
  ...
  const uGlowSpeed = useSharedValue<number>(DEFAULT_GLOW_BASE.speed);
  const uGlowSaturation = useSharedValue<number>(DEFAULT_GLOW_BASE.saturation);
  const uGlowLightness = useSharedValue<number>(DEFAULT_GLOW_BASE.lightness);
  ...
  const defaultGlowWithColors = useMemo(() => getDefaultGlow(colors), [colors]);
