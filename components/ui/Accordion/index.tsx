...
import { fonts, spacing, radius, shadows, type ColorTokens } from "@/constants/tokens";
import { useTheme } from "@/hooks/useTheme";
import { Icon } from "@/components/ui/Icon";
import { getAccordionThemes } from "./presets";
import type {
...
const ChevronIcon = ({ isOpen }: { isOpen: boolean }) => {
  ...
  return (
    <Animated.View style={animatedStyle}>
      <Icon name="chevron-down" size={20} color={theme.iconColor} />
    </Animated.View>
  );
};
...
const Accordion = ({
  ...
  const accordionThemes = useMemo(() => getAccordionThemes(colors), [colors]);
  const theme = themeProp ?? accordionThemes.contra;
  ...
  return (
    <AccordionContext.Provider
      value={{ openItems, toggleItem, theme, spacing: itemSpacing }}
    >
      <View
        style={[
          styles.accordion,
          {
            ...
            borderWidth: theme.borderColor === "transparent" ? 0 : 1,
...
const AccordionItem = ({
  ...
  const { openItems, theme, spacing: itemSpacing } = useAccordionContext();
    ...
    <AccordionItemContext.Provider value={{ value, isOpen, icon }}>
      <Animated.View
        style={[
          styles.item,
          {
            borderBottomColor: isLast ? "transparent" : theme.dividerColor,
